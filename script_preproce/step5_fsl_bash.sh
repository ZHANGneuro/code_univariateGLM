...
for sub in {7..7}; do
  ...
  for run in {1..4}; do
    ...
    TEMPLATE_PATH=$root_path"fsf_template/feat_template_prep.fsf"
    OUTPUTDIR=$root_path"file_generated/preprocessing_sm"$SMOOTH"_sub"$sub"_run"$run
    ...
    sed -e 's@OUTPUTDIR@'$OUTPUTDIR'@g' \
        ...
        -e 's@NUM_VOXEL@'$NUM_VOXEL'@g' <$TEMPLATE_PATH> $root_path"file_generated/fsf_prep_sm"$SMOOTH"_sub"$sub"_s"$run".fsf"
